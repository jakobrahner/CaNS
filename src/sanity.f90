...
module mod_sanity
  ...
  subroutine test_sanity_solver(ng,lo,hi,n,n_x_fft,n_y_fft,lo_z,hi_z,n_z,dli,dzc,dzf,dzci,dzfi,dzci_g,dzfi_g, &
    ...
    if(is_impdiff .and. .not.is_impdiff_1d) then
      ...
      call fftend(arrplan)
      call bounduvw(cbcvel,n,ng,lo,bcvel,nb,is_bound,.false.,dl,dzc,dzf,u,v,w) ! actually we are only interested in boundary condition in u
      ...
      call fftend(arrplan)
      call bounduvw(cbcvel,n,ng,lo,bcvel,nb,is_bound,.false.,dl,dzc,dzf,u,v,w) ! actually we are only interested in boundary condition in v
      ...
      call fftend(arrplan)
      call bounduvw(cbcvel,n,ng,lo,bcvel,nb,is_bound,.false.,dl,dzc,dzf,u,v,w) ! actually we are only interested in boundary condition in w
